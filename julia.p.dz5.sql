...
SELECT id, storehouse_id,product_id,value FROM storehouses_products WHERE `value` > 0 GROUP BY `value`
UNION
...
SELECT DAYOFWEEK(birthday_at+ INTERVAL TIMESTAMPDIFF(YEAR ,birthday_at, CURDATE()) YEAR) as nday,
	COUNT(DAYOFWEEK(birthday_at+ INTERVAL TIMESTAMPDIFF(YEAR ,birthday_at, CURDATE()) YEAR)) as summday
